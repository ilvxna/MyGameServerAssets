...
MSG_GET_GUILDS                     = 36          #--分页获取服务器的公会，                          请求：(参数1：开始索引Index，参数2：偏移值，参数3：无效    例子：从第1个公会开始，取10个公会，则传 1 10)； 回应lua_table：({1=数量 2={1={1=公会dbid2=名称3=等级4=人数} ...}})
...
MSG_GET_GUILD_DETAILED_INFO        = 44          #--获取公会详细信息                                       请求：(参数无效)； 回应lua_table：({1=公告2=公会资金3=公会等级4=公会人数5=公会长名称6=当前龙晶值7={技能ID=等级...}})
MSG_GET_GUILD_MESSAGES_COUNT       = 45         #--获取公会消息的数量                                  请求：(参数1：消息类型， 参数2：无效， 参数3：无效              消息类型目前只有一种，传1表示获取申请加入公会的消息类型)；回应lua_table：({1=消息数量})
MSG_GET_GUILD_MESSAGES             = 46         #--分页获取公会消息                                       请求：(参数1：开始索引Index，参数2：偏移值，参数3：消息类型(先转成字符串))； 回应lua_table：({1=消息数量2={1={1=消息dbid2=申请人名称3=申请人职业4=申请人等级5=申请人战力} ...}})
...
MSG_GET_GUILD_MEMBERS              = 60         #--获取工会成员列表                                      请求：(参数1：开始索引Index，参数2：偏移值，参数3：无效)； 回应lua_table：({1={1=玩家dbid2=姓名3=等级4=职位ID5=战斗力6=贡献度7=上线时间} ...})
...
MSG_GET_RECOMMEND_LIST             = 63         #--公会长和副公会长获取推荐成员列表     请求：(参数无效)； 回应lua_table：({1={1=玩家dbid2=姓名3=等级4=战斗力} ...})
MSG_GET_DRAGON_INFO                = 64         #--获取龙晶信息                                                  请求：(参数无效)； 回应lua_table：({1=注魔次数2=上一次注魔时间})
